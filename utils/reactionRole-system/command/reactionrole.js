			...
			try {
				if (result) {
					...
					if (checkFeaturesisEnabled?.reactionRoleSystem_enabled) {
						...
						await runDb('INSERT INTO reactionrole_system_reactions (guildId, roleId, messageId, channelId, emoji) VALUES (?, ?, ?, ?, ?)', interaction.guild.id, roleId, message, interaction.channel.id, emoji);
