...
module.exports = {
  ...
  async execute(auditLogEntry, guild) {
    ...
    database.db.get(sqlEnabledFeature, [guild.id], (_, result_Db) => {
      if (!result_Db) return;
      ...
      database.db.get(sqlChannelId_log, [guild.id], (_, result) => {
        ...
        if (guild?.id) {
          language.databaseCheck(guild.id)
            .then(data => {
              const langagues_path = readFileSync(`./languages/logs_system/${data}.json`);
              const language_result = JSON.parse(langagues_path);

              guild.channels.fetch(result.guildAuditLogs_channel)
                .then(channel_logs => {
                  const fields = [];
                  ...
                  setTimeout(() => {
                    ...
                    setTimeout(() => {
                      ...
                      const embedLog = new EmbedBuilder()
                        .setAuthor({ name: `${language_result.guildAuditLogsEntry.embed_title}` })
                        .addFields(fields)
